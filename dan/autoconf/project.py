...
class Project(Target, internal=True):
    ...
    def make(self):
        if self.__make is None:
            if self.toolchain.system.startswith("msys"):
                ...
                self.__make = find_executable(
                    r".+make", data["InstallLocation"], default_paths=False
                )
                if self.__make is None:
    ...
    async def __build__(self):
        if not self.toolchain.system.is_linux and not self.toolchain.system.startswith(
            "msys"
        ):
    ...
    async def do_compile(self, obj, options, env):
        ...
        await async_run(
            f"{self.toolchain.cc} {options}", logger=self, cwd=self.build_path, env=env
        )
    ...
    async def do_ar(self, lib, ar_arg, objects: list[str], builds, env):
        ...
        if os.name == "nt" and len(command) >= windows_command_line_limitation:
            ...
            for obj in objects:
                ...
                dest = objects_dir / obj_path.relative_to(
                    objects_dir.parent
                ).as_posix().replace("/", "_")
                if not dest.exists():
    ...
    async def do_ld(
        self, output, lib_paths, objects, libs, builds: dict[str, asyncio.Task], env
    ):
        ...
        await async_run(
            f'{self.toolchain.cc} {lib_paths} -o {output} {" ".join(objects)} {libs}',
            logger=self,
            cwd=self.build_path,
            env=env,
        )
    ...
    async def do_ranlib(self, lib: str, builds: dict[str, asyncio.Task], env):
        ...
        if lib in builds:
            ...
            if not p.is_absolute():
                p = self.build_path / p
            if not p.exists():
                self.warning("ranlib skipped: %s", p)
                return

        await async_run(
            f"{self.toolchain.ranlib} {lib}", logger=self, cwd=self.build_path, env=env
        )
    ...
    async def __install__(self, installer: Installer):
        ...
        await async_run(
            ["bash", self.source_path / "configure", *config_options],
            cwd=self.build_path,
            logger=self,
            env=env,
        )
        ...
        if self.wrap_makefile_commands:
            async with asyncio.TaskGroup(f"executing make {self.name}") as g:
                        ...
                        async for line in lines:
                            ...
                            match re_match(line):
                                case r".+CC.+? (.+\.o); (.+?) (.+)" as m:
                                    ...
                                    # cc = m[2]
                                    options = m[3]
                                    builds[obj] = asyncio.create_task(
                                        self.do_compile(obj, options, env)
                                    )
                                ...
                                case r".+AR.+? (.+\.a); (.+?) (.+?) (.+\.a) (.+)" as m:
                                    ...
                                    ar_arg = m[3]
                                    objects = m[5].split(" ")
                                    builds[lib] = asyncio.create_task(
                                        self.do_ar(lib, ar_arg, objects, builds, env)
                                    )
                                ...
                                case (
                                    ...
                                    objects = m[5].split(" ")
                                    libs = m[6]
                                    builds[output] = asyncio.create_task(
                                        self.do_ld(
                                            output,
                                            lib_paths,
                                            objects,
                                ...
                                case (
                                    r".+INSTALL.+? (.+?); install (.+(?=-m \d+))?(-m \d+) (.+)" as m
                                ):
                                    # args = m[2]
                                    # mode = m[3]
                                    items = str(m[4]).split(" ")
                                    ...
                                    for item in items:
                                        installs.append(
                                            installer._install(
                                                self.build_path / item, dest
                                            )
                                        )
                                case r".?ranlib\s+(.+)" as m:
                                    lib = m[1]
                                    builds[f"ranlib-{lib}"] = asyncio.create_task(
                                        self.do_ranlib(lib, builds, env)
                                    )
                                case _:
                ...
                await async_run(
                    [self.make, "install", "-n", *self.__get_make_args()],
                    cwd=self.build_path,
                    logger=self,
                    log=False,
                    env=env,
                    out_capture=wrap_executor,
                )
            ...
            await async_run(
                [self.make, *self.__get_make_args()],
                cwd=self.build_path,
                logger=self,
                log=True,
                env=env,
            ...
            await async_run(
                [self.make, "install", *self.__get_make_args()],
                cwd=self.build_path,
                logger=self,
                log=True,
                env=env,
