...
class GitSources(SourcesProvider, internal=True):
    ...
    def __init__(
        self, *args, url=None, refspec=None, patches=None, subdirectory=None, **kwargs
    ) -> None:
    ...
    async def __build__(self):
        try:
            ...
            await async_run(
                f"git remote add origin {self.url}", logger=self, cwd=self.output
            )
            if self.subdirectory is not None:
                # use sparse
                await async_run(
                    f"git config core.sparseCheckout true", logger=self, cwd=self.output
                )
                async with aiofiles.open(
                    self.git_dir / "info" / "sparse-checkout", "w"
                ) as f:
                    await f.writelines([self.subdirectory])

            await async_run(
                f"git fetch -q --depth 1 origin {self.refspec}",
                logger=self,
                cwd=self.output,
            )
            ...
            for patch in self.patches:
                await async_run(
                    f"git am {self.source_path / patch}", logger=self, cwd=self.output
                )
