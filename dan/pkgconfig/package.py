...
def find_pkg_config(
    name, paths: str | Path | list[str | Path] = list(), use_default_paths=True
) -> Path:
    ...
    return find_file(rf"(lib)?{re.escape(name)}\.pc$", paths, re.IGNORECASE)


def find_pkg_configs(
    name, paths: str | Path | list[str | Path] = list(), use_default_paths=True
    ...
    if use_default_paths:
        ...
        paths = [*paths, *_get_pkg_config_paths()]
    yield from find_files(rf"(lib)?{re.escape(name)}\.pc$", paths, re.IGNORECASE)
    ...
    def get(self, name: str, default=None):
        ...
        if type(value) == str:
            while True:
                ...
                if m:
                    var = m.group(1)
                    value = value.replace(f"${{{var}}}", self.get(var))
                else:
                    break
        return value
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if dan_plugin:
            ...
            self.__dan_plugin = find_file(
                rf"{name}\.py", [self.config_path.parent.parent]
            )
        else:
    ...
    async def __initialize__(self):
        ...
        if reqs:
            async with asyncio.TaskGroup(
                f"resolving {self.name}'s requirements"
            ) as group:
                for req in reqs:
                    if req.name in self.all:
                        if not req.is_compatible(self.all[req.name]):
                            raise RuntimeError(
                                f"Incompatible requirements detected {req} vs {self.all[req.name]}"
                            )
...
def get_packages_cache(context) -> dict[str, PackageConfig]:
    ...
    if pkgconfig_cache is None:
        context.root.build_path.mkdir(exist_ok=True, parents=True)
        pkgconfig_cache = Cache.instance(
            context.root.build_path / "pkgconfig.cache",
            cache_name="pkgconfig",
            binary=True,
        )
...
def find_package(
    ...
    if name in cache:
        ...
        if cached_pkg.config_path.exists():
            if spec and not spec.is_compatible(cached_pkg.version):
                raise RuntimeError(
                    f"incompatible package {name} ({cached_pkg.version} {spec})"
                )
            return cached_pkg
        else:
            cached_pkg.warning("cached config does not exist anymore")
            del cache[name]

    search_paths = unique(search_paths or makefile.env.package_search_paths)
    for config in find_pkg_configs(
        name, search_paths, use_default_paths=use_default_paths
    ):
        if spec is not None:
            ...
            pkg = PackageConfig(
                name, config_path=config, search_paths=search_paths, makefile=makefile
            )
            break
...
def _get_jinja_env():
    ...
    if _jinja_env is None:
        _jinja_env = jinja2.Environment(loader=jinja2.PackageLoader("dan.pkgconfig"))
    return _jinja_env
...
async def create_pkg_config(lib: Library, settings: InstallSettings) -> Path:
    ...
    for dep in lib.dependencies.public:
        match dep:
            case RequiredPackage():
                if dep.version_spec:
                    requires.append(
                        f"{dep.name} {dep.version_spec.op} {dep.version_spec.version}"
                    )
                else:
    ...
    if not lib.interface:
        libs.extend(
            lib.toolchain.make_libpath_options([Path(f"${{libdir}}/{lib.name}")])
        )
        libs.extend(lib.toolchain.make_link_options([Path(f"${{libdir}}/{lib.name}")]))
