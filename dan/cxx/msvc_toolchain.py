...
class MSVCToolchain(Toolchain):
    ...
    async def scan_dependencies(
        self, sourcefile: Path, output: list[str], options: set[str]
    ) -> set[FileDependency]:
    ...
    def make_compile_commands(
        ...
        args = [
            self.cc,
            *unique(
                self.common_flags, self.default_cflags, self.default_cxxflags, options
            ),
            "/sourceDependencies",
            deps,
            f"/Fo{str(output)}",
    ...
    def make_link_commands(
        self, objects: set[Path], output: Path, options: list[str]
    ) -> CommandArgsList:
        return [
            [self.lnk, *self.common_flags, *options, *objects, f"/OUT:{str(output)}"]
        ]
    ...
    def make_shared_lib_commands(
        ...
        return [
            [
                self.lnk,
                *self.common_flags,
                f'/IMPLIB:{output.with_suffix(".lib")}',
                "/DLL",
                *options,
                *objects,
                f'/OUT:{output.with_suffix(".dll")}',
            ]
        ]
    ...
    async def _handle_compile_output(self, lines) -> t.Iterable[diag.Diagnostic]:
        async for line in lines:
            ...
            match re_match(line):
                case (
                    r"(.+)\((\d+)\):\s+(?:fatal\s+)?(error|warning)\s(\w+\d+):\s(.+)$" as m
                ):
        ...
        async for line in lines:
            match re_match(line):
                case r"(.+?)\s?:\serror\s(\w+\d+):\s(.+)$" as m:
                    yield diag.Diagnostic(
                        message=m[3].strip(), code=m[2], source=self.type, filename=m[1]
                    )
                case r"LINK\s?:\s?fatal\s+error\s+(\w+\d+):\s+(.+)$" as m:
                    yield diag.Diagnostic(
                        message=m[2].strip(), code=m[1], source=self.type
                    )
                case _:
                    self._logger.debug("Unhandled line: %s", line)
