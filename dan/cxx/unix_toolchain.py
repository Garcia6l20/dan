...
from dan.core.toolchains import Lang
from dan.core import aiofiles, diagnostics as diag, asyncio
...
class UnixToolchain(Toolchain):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.debug("C compiler is %s %s (%s)", self.type, self.version, self.cc)
        self.debug("C++ compiler is %s %s (%s)", self.type, self.version, self.cxx)
    ...
    def get_base_compile_args(
        ...
        match lang.name:
            case "c++":
                return [
                    self.cxx,
                    *self.get_optimization_flags(build_type),
                    f"-std=c++{cpp_std or self.cpp_std}",
    ...
    def make_compile_commands(
        ...
        args = self.get_base_compile_args(
            sourcefile=sourcefile, build_type=build_type, cpp_std=cpp_std
        )
        args.extend(
            [
    ...
    def make_link_commands(
        ...
        commands = [args]
        if build_type in [BuildType.release, BuildType.release_min_size]:
    ...
    def make_shared_lib_commands(
        ...
        args = [
            ...
            "-shared",
            *objects,
            *unique(self.default_ldflags, options),
            "-o",
            output,
        ]
        commands = [args]
    ...
    async def get_default_include_paths(self, lang=None) -> list[Path]:
        if lang is None:
            lang = "c++"
        ...
        if includes is None:
            ...
            out, err, rc = await self.run(
                f"get default {lang} includes",
                None,
                args,
                quiet=True,
                log=False,
    ...
    # def get_extra_detect_flags(self, lang=None):
    #     includes = asyncio.sync_wait(self.get_default_include_paths(lang))
    #     includes.append("-include <bits/version.h>")
    #     return self.make_include_options(includes)
            ...
            match re_match(line):
                ...
                case r"((?:.+)from (.+)):(\d+)[,:]" as m:
                    ...
                    prev = info = diag.RelatedInformation(
                        location=diag.Location(
                            diag.Uri(filename),
                            range=diag.Range(
                                start=diag.Position(lineno), end=diag.Position(lineno)
                            ),
                        ),
                        message=message,
                    )
                ...
                case r"(.+?):(\d+):(?:(\d+):)?\s+(required from\s.+)" as m:
                    ...
                    prev = info = diag.RelatedInformation(
                        location=diag.Location(
                            diag.Uri(filename),
                            range=diag.Range(
                                start=diag.Position(lineno, character),
                                end=diag.Position(lineno, character),
                            ),
                        ),
                        message=message,
                    )
                ...
                case r"(.+?):(\d+):(?:(\d+):)?\s(note):\s(.+)$" as m:
                    ...
                    if prev_diag is None:
                        ...
                        prev = info
                case (
                    r"(.+?):(\d+):(?:(\d+):)?\s(?:fatal )?(error|warning):\s(.+)$" as m
                ):
                    character = int(m[3]) if m[3] else 0
                    lineno = int(m[2]) - 1
                    message = m[5]
                    prev_diag = prev = diag.Diagnostic(
                        message=message,
                        range=diag.Range(
                            start=diag.Position(line=lineno, character=character),
                            end=diag.Position(line=lineno),
                        ),
        ...
        async for line in lines:
            match re_match(line):
                ...
                case r"(?:.+: )?(?:(.+):)?\((.+)\+(.+)\): (.+)$" as m:
                    ...
                    message = m[4].strip()
                    yield diag.Diagnostic(message=message, source=self.type)
