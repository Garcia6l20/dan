...
class TermStream:
    ...
    async def status(self, msg: str, icon: str = None, timeout=None):
        async with asyncio.async_lock(self._lock):
            self._status = msg
            if icon is not None:
                self._icon = icon
            self._visible = True
...
class TermManager:
    def __init__(self, max_update_time=1) -> None:
        ...
        self._up_cv = threading.Condition(self._lock)
    ...
    async def write(self, s: str):
        async with asyncio.async_lock(self._up_cv):
    ...
    def _render(self):
        ...
        with ts.hidden_cursor():
            while not stop:
                with self._up_cv:
                    self._up_cv.wait()  # self._wait_timeout)
                    ...
                    for stream in self._streams:
                        ...
                        if is_dirty or force:
                            ...
                            if is_visible:
                                out.write(str(stream) + ts.next())
                                prev_line_count += 1
                            else:
                                str(stream) # flush dirty state
                        elif is_visible:
                            # just jump over
                            prev_line_count += 1
                            out.write(ts.next())
