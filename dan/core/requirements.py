...
class RequiredPackage(Logging):
    ...
    def is_compatible(self, t: "Target"):
        if self.version_spec is not None and t.version is not None:
...
async def load_requirements(
    requirements: t.Iterable[RequiredPackage],
    makefile,
    name=None,
    logger=None,
    install=True,
    ...
    async with asyncio.TaskGroup(f"loading {name} requirements") as group:
        for req in requirements:
            ...
            if t and not t.is_requirement and req.is_compatible(t):
                logger.debug("%s: already fulfilled by %s", req, t.fullname)
                req.target = t
                resolved.append(req)
                # cache_data.resolved(req)
                continue

            t = find_package(
                req.name,
                req.version_spec,
                search_paths=pkgs_search_paths,
                makefile=makefile,
            )
            ...
            if t is None and install:
                with makefile.context:
                    t, is_new = await IoPackage.instance(
                        req.name,
                        req.version_spec,
                        package=req.package,
                ...
                if is_new:
                    ...
                    logger.debug(
                        "%s: package already being installed at version %s",
                        req,
                        t.version,
                    )
