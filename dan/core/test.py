...
class Test(Logging, MakefileRegister, internal=True):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.fullname is None:
            self.fullname = (
                f"{self.context.name}{self.executable.makefile.fullname}.{self.name}"
            )
    ...
    async def _run_test(self, caze: Case):
        ...
        out, err, rc = await self.executable.execute(
            *args, no_raise=True, cwd=self.workingDir
        )
        out_log, out_err = self.outs(caze)
        async with (
            aiofiles.open(out_log, "w") as outlog,
